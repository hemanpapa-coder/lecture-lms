...
async function run() {
    ...
    // board_questions table uses string for week_name? wait, let's look at assignments
    const { data: a1, error: e1 } = await supabase.from('assignments').update({ week_name: '6주차' }).eq('user_id', user.id).eq('week_name', '3주차').select();
    if(e1) console.error(e1);
    console.log('Assignments updated:', a1?.length || 0);

    const { data: b1 } = await supabase.from('board_questions').update({ week_number: 6 }).eq('user_id', user.id).eq('week_number', 3).select();
    const { data: b2 } = await supabase.from('board_questions').update({ week_number: 6 }).eq('user_id', user.id).eq('week_number', '3주차').select();
    
    console.log('Board (num) updated:', b1?.length || 0);
    console.log('Board (string as 3주차) updated:', b2?.length || 0);
}
run();
