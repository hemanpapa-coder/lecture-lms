...
function getLocalLectureModelPlan(systemPrompt: string, userContent: string, requestedModel: string): LocalLectureModelPlan[] {
  ...
  const isDetailedScribe = /강의 속기사|내용 요약, 압축, 생략 금지|내용 임의 생략 금지|전공 서적|교재형 강의 노트|학술 작가/.test(systemPrompt)
...
function isWeakLectureSummary(output: string, systemPrompt: string): boolean {
  ...
  const isPreservationScribe = /강의 속기사|내용 요약, 압축, 생략 금지|내용 임의 생략 금지/.test(systemPrompt)
  ...
  if (isPreservationScribe) return false
...
- 내용 요약, 압축, 생략 금지
- 어떤 내용도 버리지 말 것
- 강의에 없는 새 주제나 해석을 만들어내지 말 것
- 내부 처리 문구("AI 정리 실패", "임시 정리", "응답 실패" 등)를 출력하지 말 것

[해야 할 것]
- "어", "음", "그니까", "뭐", "저" 같은 말버릇만 제거
- 구어체를 자연스러운 문어체로 변환
- 같은 내용의 완전한 반복만 1번으로 정리
- 교수님이 든 모든 예시, 경험담, 비유, 부연설명 포함
- 과제, 제출물, 다음 수업 준비, 출석/보강/수업 운영 안내 포함
- 문단 구분을 추가하고 소제목을 붙이되 내용은 그대로 유지
- 긴 문장은 의미 단위로 나누어 자연스럽게 정리하되, 길다는 이유로 삭제하지 말 것
...
async function processDetailed(
  ...
  for (let batchStart = 0; batchStart < textChunks.length; batchStart += PARALLEL) {
    ...
    await Promise.all(batchIndices.map(async (i) => {
      ...
      try {
        const result = await callTextModel(SCRIBE_SYSTEM, `아래 강의 전사 텍스트를 정서하세요. 요약하지 말고, 말버릇과 완전한 중복만 정리하세요:\n\n${textChunks[i]}`, provider, apiKey, model)
        sections[i] = result
      } catch (err: any) {
...
- 강의에 없는 새 주제나 해석을 만들어내지 말 것
- "더 궁금하면 질문해주세요", "도움이 필요하면 알려주세요" 같은 AI 챗봇식 안내 문장 금지
- "AI 정리 실패", "전사 기반 임시 정리", "로컬 AI 응답 실패" 같은 내부 처리 문구 금지
...
- 과제, 제출물, 다음 수업 준비, 출석/보강/수업 운영 안내 포함
- 논리적 흐름으로 소제목 붙여 구조화
- 번호가 붙은 큰 주제는 각각 별도 <h2>로 분리하고, 한 문단 안에 여러 번호 주제를 이어 쓰지 말 것
${VISUAL_INSTRUCTIONS}
${CONTEXT_BLOCK}
[출력 형식] 순수 HTML. html/head/body 태그 없음.
<h1>📚 강의 전체 정리</h1>
<h2>주제 섹션</h2><h3>소주제</h3><p>내용</p>
<h2>과제 및 수업 운영 안내</h2><ul><li>안내 사항</li></ul>
<h2>✅ 핵심 정리</h2><ul><li>포인트</li></ul>
...
${CONTEXT_BLOCK}
[작성 지침]
- 말버릇과 완전한 반복은 제거하되, 중요한 예시와 과제 안내는 유지하세요.
