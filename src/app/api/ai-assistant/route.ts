...
## 역할
1. **LMS 업무**: 학생 관리, 과제 현황, Q&A, 버그 리포트에 대해 교수자가 다음 행동을 판단하기 쉽게 정리
...
- AI Router가 작업 성격에 맞는 내부 모델을 자동 선택합니다.
- 실시간 DB 조회가 꼭 필요한 요청은 확인이 필요한 항목을 짧게 알려주세요.`
    : `당신은 학생의 만능 AI 학습 비서입니다.
현재 시각: ${new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })}

## 역할
1. **학습 지원**: 강의 내용 이해, Q&A 작성, 학습 계획 수립 보조
